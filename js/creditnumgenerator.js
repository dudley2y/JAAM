...
var decimal="";
var ccNum=""
function generate(ip, master){
    var message=ip+","+master;
    var sha256=hash(message);
    var all=sha256.split("");
    for(x=0;x<4;x++){
        for(y=16*x;y<16*x+16;y++){
            decimal+=all[y];
        }
    decimal=parseInt(decimal,16);
    if (decimal>9999999999999999999){
        decimal=decimal/10;
    }
    else if (decimal<999999999999999999){
        decimal=decimal*10;
    }
    decimal=Math.floor(decimal/1000000000000000)
    ccNum+=decimal.toString();
    decimal="";
    }
    return ccNum;
}
...
function luhn (sixteen){
    var sixteen=sixteen.split("");
    var sum = 0;
    var alternate=true;
    for(x=14;x>=0;x--){
        newNbrs[x]=parseInt(sixteen[x]);
        var odds= newNbrs[x];
        if(alternate){
            odds*=2;
            if (odds>9){
    ...
    newNbrs[15]=0;
    newNbrs[15]+=(10-((sum%10)));
    var final = newNbrs.join('');
    return final;
}
...
function create (ip,master){
    newCard=luhn(generate(ip,master));
    return newCard
}

var newCard=create(location.host,4430450076188613);
console.log(newCard);
